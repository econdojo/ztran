...
% Focus on Equilibrium:   Non-degenerate wealth dynamics, no fiscal transfer
...
while AllDiff >ITERATE_ACCURACY
   ...
   if infor == 1
     [Ecz_1,~] = wh(z,beta_1,Sig1_arma, preCz_arma,V,350);  % Use 350 points to increase speed
   end
   if infor == 0
   ...
   if infor == 1
     [Ecz_2,~] = wh(z,beta_2,Sig2_arma, preCz_arma,V,350); 
   end
   ...
   Erz_2 = Erz_2*beta_2;
    ...
    for k = 1:nz
        ...
        %Cz_temp(:,:,k) = (pi_1*(Ecz_1(1,1:2,k)-Erz_1(1,1:2,k))+pi_2*(Ecz_2(1,1:2,k)-Erz_2(1,1:2,k)));
         Cz_temp(:,:,k) = (1/(1-theta*z(k)))*(pi_1*(1-w_2*z(k))*(Ecz_1(1,1:2,k)-Erz_1(1,1:2,k))+pi_2*(1-w_1*z(k))*(Ecz_2(1,1:2,k)-Erz_2(1,1:2,k)));
   ...
   S_1z = zeros(nx,ne-1,nz); % group 1 wealth
   S_2z = zeros(nx,ne-1,nz); % group 2 wealth
   
   for k = 1:nz
       S_1z(:,:,k) = (1/(1-w_1*z(k)))*(phi_1*Cz(:,:,k)+Erz_1(1,1:2,k)-Ecz_1(1,1:2,k));
...
if infor == 0 
 C_full_impact_4 = C_agg_1(1);
 save('normaliz_4','C_full_impact_4');
end

load('normaliz_4');
C_agg_1 = C_agg_1/C_full_impact_4;
C_agg_2 = C_agg_2/C_full_impact_4;
...
subplot(2,2,3)
s0_imp = plot(0:PERIODS-1,S1_agg_1,'-b',0:PERIODS-1,S2_agg_1,'-g','linewidth',1.2);
title('Fundamental Shock','Interpreter','latex','Fontsize',12);
set(s0_imp,'LineWidth',2);
xlabel('Periods','Interpreter','latex','Fontsize',12);
%ylabel('$(\%)$','Interpreter','latex','Rotation',0);

legend('Group 1 Wealth','Group 2 Wealth','Interpreter','latex','Fontsize',7);
%legend('Hank-Network Model, Full Information','Interpreter','latex');

subplot(2,2,4)
s1_imp = plot(0:PERIODS-1,S1_agg_2,'-b',0:PERIODS-1,S2_agg_2,'-g','linewidth',1.2);
title('Group 1 Noise Shock','Interpreter','latex','Fontsize',12);
set(s1_imp,'LineWidth',2);
xlabel('Periods','Interpreter','latex','Fontsize',12);
%ylabel('$(\%)$','Interpreter','latex','Rotation',0);
...
xlabel('Periods','Interpreter','latex','Fontsize',12);
%ylabel('$(\%)$','Interpreter','latex','Rotation',0); 
legend('FI + Welath Dynamics','Interpreter','latex','Fontsize',7);

subplot(1,2,2)
s0_imp = plot(0:PERIODS-1,S1_agg_1,'-b',0:PERIODS-1,S2_agg_1,'-g','linewidth',1.2);
title('Wealth Dynamics: Fundamental Shock','Interpreter','latex','Fontsize',12);
set(s0_imp,'LineWidth',2);
xlabel('Periods','Interpreter','latex','Fontsize',12);
%ylabel('$(\%)$','Interpreter','latex','Rotation',0);
