% This code solves a HANK model with asymmetric incomplete information in
...
phi1 = 1.99;        % group 1's exposure to business cycle
phi2 = 0.01;        % group 2's exposure to business cycle
...
sig_xi = 10;        % std. dev. of private endogenous signal
...
xm1i = 10;  % group 1's endogenous signal
...
s1i_m = 4;  % shock to group 1's individual endogenous signal
...
% Eqn 10: group 1's endogenous signal
Ax0(10, xm1i) = 1;
Ax0(10, xy) = -1;
As0(10, s1i_m) = -1;

% Specify shocks
% AR(1) real interest rate shock
Cs1(sr,sr) = rho_r;
Ds0(sr,er) = 1;
...
if group_index == 1
    % group 1 receives endogenous signal
    ...
    % group 2 receives endogenous signal
else
    sig = {                              % {eqn_id, end_id, exo_id, ave_ex}
        6, [], s1i, false
        7, xm1i, s2i, false
...
% IRFs
T = 40;
var_lab = {'$y$', '$c_1$', '$c_2$', '$s_1$', '$s_2$', ...
    '$x_1^i$', '$x_2^i$', '$z_1^i$', '$z_2^i$', '$m_1^i$'};
inn_lab = {'$\varepsilon_r$', '$\eta_1^i$', '$\eta_2^i$', '$\xi_1^i$'};
