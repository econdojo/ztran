...
% Solve for PI equilibrium 1 where idiosyncratic consumption and capital features random walk, aggregate equilibrium
% stationary
...
h_be = (beta-1)*polyval(Paz,beta)/polyval(Pcz,beta);
b_be = sig_z/(1-rho_z*beta);
c_t = h_r2*a_r2;
c_b = h_be*b_be;

M_hl_1 = (1-tau_a)*conv([-beta 1], [-1 r_2]);
M_hl_2 = tau_a*conv([-r_2 1],[-1 beta]);
M_hl = sum_poly_coeff(M_hl_1,M_hl_2);

N_hl_1 = c_b*tau_b*conv([-beta 1], [-1 r_2]);
N_hl_2 = c_t*tau_a*conv([-r_2 1],[-1 beta]);
N_hl = sum_poly_coeff(N_hl_1,N_hl_2);

fl_1 = sig_a*conv(Paz,[1 -1]);
fl_1 = conv(fl_1,M_hl);
fl_2 = conv(Pcz,[-rho_a 1]);
fl_2 = conv(fl_2,N_hl);
...
% Functional constant
M_l = polyval(F_12z,r_2)*polyval(F_12z,beta)-polyval(F_22z,beta)*polyval(F_11z,r_2); % function of lambda
theta_1 = (c_b*lambda*(1-lambda*beta)*polyval(F_12z,r_2)-c_t*lambda*(1-lambda*r_2)*polyval(F_22z,beta))/M_l;
theta_2 = (c_b*lambda*(1-lambda*beta)*polyval(F_11z,r_2)-c_t*lambda*(1-lambda*r_2)*polyval(F_12z,beta))/M_l;
...
Ciz = zeros(1,1,nz); % stationary part
...
for i = 1:nz
    ...
    Ciz(1,1,i) = (1/sig_z)*(polyval(F_12z, z(i))*theta_1-polyval(F_22z,z(i))*theta_2)/(lambda*(1-lambda*z(i)));
end
...
i2 = varma.fit(z,Ciz,p,q);
imp = zeros(1,PERIODS);
imp(1) = 1;
res_c = irf(c2,imp,1e-4);
res_r = irf(r2,imp,1e-4);
res_i = irf(i2,imp,1e-4);
...
legend('PI Equilibrium 1, Closed-form Solution','Interpreter','latex');
